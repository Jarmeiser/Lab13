...
public class CalculatorFrame extends JFrame {
	...
	public CalculatorFrame(String title) {
		...
		// added into it.
		// TODO: finish adding elements into the panel. Reference the writeup to see
		...
		panel0.add(slider);
		// TODO: add the panels into the frame. Remember that this class is a JFrame, so
		...
		/*
		 * When a button is pressed it should: (1) change the opSign text to the name of
		 * the respective radio Button (+, *, /, ==) (2) clear the computeResult text
		 * field (3) clear the errorMessage text field
		 * 
		 * Note: pressing the button sets it to be selected. Because the buttons are in
		 * a button group, when one button is selected all other buttons will be
		 * deselected. As such, you should not add any code to explicitly set the
		 * buttons to be selected.
		 * 
		 * ActionListeners wait for the program to send them an event. In this case, an
		 * event occurs when we press a button. Once that event happens, the code inside
		 * the actionPerformed method is executed. Thus, we are coding here what the
		 * program should do when a button is pressed. We add action listners to each of
		 * the radio buttons.
		 * 
		...
		// TODO: implement the actionlisteners for the divide and equality buttons the
		// same way:

		/*
		 * Set the Change Listener for the Slider. When a change occurs, the text in
		 * firstBox should be set to the slider value.
		 * 
		 * The text in the computeResult and errorMessage should also be cleared.
		 * 
		 * ChangeListener is a type of listener like ActionListener. Events are sent to
		 * it when the slider is changed. The method called is stateChanged instead of
		 * actionPerformed for this listener.
		 */
		...
		/*
		 * When the Calculate Result Button is pressed, get the values from the text
		 * fields and calculate the appropriate result using the four different
		 * operators (+, *, /, ==).
		 * 
		 * You need to check which operation (button) is selected to decide what to do.
		 * You may need to look at the JRadioButton API to determine how best to do
		 * this.
		 * 
		 * (1) + operation: add the integer values from firstBox and secondBox. Set
		 * computeResult to the result. (2) - operation: subtract the integer value of
		 * secondBox from firstBox. Set computeResult to the result. (3) / operation:
		 * divide the integer value of firstBox by secondBox. Set computeResult to the
		 * result. (4) == operation: if the integer value of firstBox == secondBox, set
		 * computeResult to the string "True". Else, set computeResult to the string
		 * "False".
		 * 
		 * You should reset errorMessage when the computation succeeds. You should reset
		 * computeResult when you encounter an error.
		 */
		button.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					/*
					 * TODO: determine which radio button is selected (add, multiply, divide,
					 * equality).
					 * 
					 ...
					 * 
					 * Finally, perform the operation on the integers and write out the result as a
					 * String to the computeResult text field.
					 */
					
					if(add.isSelected()) {
						Integer result = Integer.parseInt(firstBox.getText())+ Integer.parseInt(secondBox.getText());
						computeResult.setText(result.toString());
					}
					...
					if(equality.isSelected()) {
						Boolean result = Integer.parseInt(firstBox.getText())== Integer.parseInt(secondBox.getText());
						computeResult.setText(result.toString());
					}
